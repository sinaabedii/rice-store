...
import { FiChevronRight, FiChevronLeft, FiStar, FiMinus, FiPlus, FiShoppingCart, FiShare2, FiHeart } from 'react-icons/fi';
import ProductCard from '@/components/ui/ProductCard';
import { useCartStore } from '@/store/cartStore';

interface ProductDetailsProps {
  product: any; // در یک پروژه واقعی، باید تایپ‌های مناسب تعریف شوند
}
    ...
    <div>
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <Slider {...thumbnailSettings}>
                {product.images.map((img: string, index: number) => (
                  <div key={index} className="px-1 cursor-pointer">
                    <div 
                      className={`relative aspect-square rounded-md overflow-hidden border-2 ${activeImage === index ? 'border-primary' : 'border-transparent'}`}
          ...
          {/* Product Info */}
          <div>
            <h1 className="text-2xl font-bold text-accent mb-2">{product.title}</h1>
            ...
            <div className="flex items-center mb-4">
              ...
              <span className="text-gray-500 text-sm">{product.reviews.length} نظر</span>
            </div>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xl font-bold text-primary">{product.price.toLocaleString()} تومان</span>
              </div>
              
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-500">
                  موجودی: {product.stock > 10 ? 'موجود' : `تنها ${product.stock} عدد باقی مانده`}
                </span>
                {product.stock > 0 ? (
            ...
            <p className="text-sm text-gray-500">
              ...
              <br />
              • ضمانت اصالت و کیفیت تمامی محصولات
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mt-8">
        <div className="border-b mb-6">
          <ul className="flex flex-wrap -mb-px">
            <li className="ml-4">
              <button
                className={`inline-block py-2 px-4 border-b-2 ${activeTab === 'description' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
            ...
            <li className="ml-4">
              <button
                className={`inline-block py-2 px-4 border-b-2 ${activeTab === 'specifications' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
            ...
            <li>
              <button
                className={`inline-block py-2 px-4 border-b-2 ${activeTab === 'reviews' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
        ...
        <div>
            ...
            <div>
              <ul className="border rounded-lg overflow-hidden divide-y">
                {product.specifications.map((spec: any, index: number) => (
                  <li key={index} className={`flex ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}>
                    <span className="px-4 py-3 w-1/3 font-medium bg-gray-100">{spec.name}</span>
          ...
          {activeTab === 'reviews' && (
            <div>
              ...
              <div>
                <h3 className="font-semibold text-lg mb-4">نظر خود را ثبت کنید</h3>
                ...
                <form>
                  ...
                  <div className="mb-4">
                    <label htmlFor="review" className="block text-gray-700 mb-2">متن نظر</label>
                    <textarea
                      id="review"
                      rows={4}
